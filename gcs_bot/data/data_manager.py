...
class DataManager:
    def __init__(self, db_manager, config, logger):
        ...
        self.config = config
        self.logger = logger
        self.query_api = self.db_manager.get_query_api()
    ...
    def read_data_from_influx(self, measurement: str, start_date: str, end_date: str = "now()") -> pd.DataFrame:
        ...
        try:
            self.logger.info(f"Executando query na measurement '{measurement}' de {start_date} até {end_date}...")
            result = self.query_api.query_data_frame(query=flux_query)

            if result.empty:
                self.logger.warning(f"Nenhum dado encontrado para a measurement '{measurement}' no período especificado.")
                return pd.DataFrame()
            ...
            self.logger.info(f"{len(df)} registros carregados da measurement '{measurement}'.")
            return df
