...
class TestCapitalManager(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_config_manager.getboolean.side_effect = self.get_config_value
    ...
    def get_config_value(self, section, key=None, fallback=None):
        # Handle getboolean calls which may not have a section
        if key is None: # Call is getboolean('STRATEGY_RULES', 'use_dynamic_difficulty', fallback=False)
            key = section
        if key == 'use_dynamic_difficulty':
            return self.config_values.get(key, fallback)
    ...
    def test_hard_limit_when_dynamic_difficulty_is_off(self):
        ...
        self.config_values['use_dynamic_difficulty'] = False
    ...
    def test_scaling_difficulty_when_dynamic_difficulty_is_on(self):
        ...
        self.config_values['use_dynamic_difficulty'] = True
        self.capital_manager = CapitalManager(self.mock_config_manager, self.mock_strategy_rules)
        ...
        self.mock_strategy_rules.evaluate_buy_signal.assert_called_with({}, 12, 2)
