...
class BacktestingEngine:
    """A class to run the backtest."""

    def __init__(self, model: Any, scaler: Any, test_data_with_features: pd.DataFrame, strategy_params: Dict[str, Any], feature_names: List[str]):
        """
        Initializes the BacktestingEngine class.
        ...
        self.model = model
        self.scaler = scaler
        self.test_data_with_features = test_data_with_features
        self.strategy_params = strategy_params
        self.feature_names = feature_names

    def run(self) -> Tuple[float, float, float, int, float, float, float]:
        """
        Runs the backtest.
        ...
        stop_loss_atr_multiplier = self.strategy_params.get('stop_loss_atr_multiplier', 2.5)
        trailing_stop_multiplier = self.strategy_params.get('trailing_stop_multiplier', 1.5)
        aggression_exponent = self.strategy_params.get('aggression_exponent', 2.0)
        max_risk_scale = self.strategy_params.get('max_risk_scale', 3.0)
        min_risk_scale = self.strategy_params.get('min_risk_scale', 0.5)
        profit_threshold = self.strategy_params.get('profit_threshold', 0.015)
        treasury_allocation_pct = self.strategy_params.get('treasury_allocation_pct', 0.20)

        in_position, buy_price, position_phase, current_stop_price, highest_price_in_trade = False, 0.0, None, 0.0, 0.0

        X_test_features = self.test_data_with_features[self.feature_names].fillna(0)
        X_test_scaled = pd.DataFrame(self.scaler.transform(X_test_features), index=X_test_features.index, columns=X_test_features.columns)
        predictions_buy_proba = pd.Series(self.model.predict_proba(X_test_scaled)[:, 1], index=X_test_features.index)
        ...
        confidence_manager = AdaptiveConfidenceManager(
            ...
            window_size=self.strategy_params.get('confidence_window_size', 5),
            pnl_clamp_value=self.strategy_params.get('confidence_pnl_clamp', 0.02),
            reactivity_multiplier=self.strategy_params.get('reactivity_multiplier', 5.0) # <-- PARÂMETRO ADICIONADO
        )

        for date, row in self.test_data_with_features.iterrows():
            price = row['close']
...
def run_backtest(model: Any, scaler: Any, test_data_with_features: pd.DataFrame, strategy_params: Dict[str, Any], feature_names: List[str]) -> Tuple[float, float, float, int, float, float, float]:
    backtesting_engine = BacktestingEngine(model, scaler, test_data_with_features, strategy_params, feature_names)
    return backtesting_engine.run()
    initial_capital = 100.0
    capital_usdt = initial_capital
    trading_btc = 0.0
    long_term_btc_holdings = 0.0
