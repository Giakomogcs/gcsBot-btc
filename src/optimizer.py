# src/optimizer.py (VERSÃO 8.0 - Com Especialista Generalista de Fallback)
...
class WalkForwardOptimizer:
    def __init__(self, full_data, feature_names):
        ...
        self.feature_names = feature_names # Recebe a lista de features do DataManager
    ...
    def _save_final_metadata(self):
        try:
            ...
            metadata = {
                ...
                'feature_names': self.feature_names, # A lista de features agora é fixa
                'optimization_summary': self.optimization_summary
            }
            with open(MODEL_METADATA_FILE, 'w') as f: json.dump(metadata, f, indent=4)
            logger.info(f"✅ Metadados salvos. Conjunto de modelos válido até {valid_until.strftime('%Y-%m-%d')}.")
        except Exception as e: logger.error(f"❌ Falha ao salvar metadados finais: {e}")
    ...
    def _progress_callback(self, study, trial):
        ...
        with lock:
            ...
            try:
                with open(OPTIMIZER_STATUS_FILE, 'w') as f: json.dump(status_data, f)
            except Exception as e: logger.warning(f"Não foi possível escrever o arquivo de status: {e}")
    ...
    def _objective(self, trial, data_for_objective):
        if self.shutdown_requested: raise optuna.exceptions.TrialPruned("Shutdown solicitado.")
        ...
        if not all_fold_metrics: raise optuna.exceptions.TrialPruned("Nenhum fold produziu resultados.")
        ...
        total_trades = metrics_df['trade_count'].sum()

        if total_trades < 10: raise optuna.exceptions.TrialPruned(f"Total de trades ({total_trades}) baixo.")
        ...
        if math.isnan(score_principal) or score_principal < 0.1: raise optuna.exceptions.TrialPruned("Score baixo.")
        logger.debug(f"Trial {trial.number} concluído. Score: {score_principal:.4f}")
    ...
    def run_optimization_for_specialist(self, name: str, data: pd.DataFrame):
        ...
        if best_score > 0.33:
            ...
            final_model, final_scaler = self.trainer.train(data, study.best_params, self.feature_names)
            ...
            strategy_params = {k: v for k, v in study.best_params.items() if k not in LGBMClassifier().get_params()}
            with open(os.path.join(DATA_DIR, params_filename), 'w') as f: json.dump(strategy_params, f, indent=4)

            self.optimization_summary[name] = {'status': 'Optimized and Saved', 'score': best_score, 'model_file': model_filename, 'params_file': params_filename, 'scaler_file': scaler_filename}
            log_table(f"Melhores Parâmetros para {name}", {k: [v] for k, v in study.best_params.items()}, headers="keys")
        else:
            logger.warning(f"❌ Score de '{name}' ({best_score:.4f}) não atingiu o limiar de qualidade (0.5).")
            self.optimization_summary[name] = {'status': 'Skipped - Low Score', 'score': best_score}

    def run(self):
        logger.info("\n" + "="*80 + "\n--- 🚀 INICIANDO PROCESSO DE OTIMIZAÇÃO (V8.0) 🚀 ---\n" + "="*80)
        ...
        for base, sub_regimes in regime_groups.items():
            ...
            data_for_single_regime = recent_data[recent_data['market_regime'] == sub_regimes[0]]
        ...
        for name, data in tasks_to_run.items():
            if self.shutdown_requested:
                logger.warning("Otimização interrompida."); break
            self.run_optimization_for_specialist(name, data)

        if not self.shutdown_requested: self._save_final_metadata()
        ...
        if os.path.exists(OPTIMIZER_STATUS_FILE): os.remove(OPTIMIZER_STATUS_FILE)
        if os.path.exists(OPTIMIZER_STATUS_LOCK_FILE): os.remove(OPTIMIZER_STATUS_LOCK_FILE)