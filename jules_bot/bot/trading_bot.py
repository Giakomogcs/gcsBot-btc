...
class LivePortfolioManager:
    ...
    def get_total_portfolio_value(self, current_price: Decimal, force_recalculation: bool = False) -> Decimal:
        # In backtest mode, this method is mocked, so this logic only runs in live/test modes.
        ...
        if force_recalculation or (current_time - self.last_recalculation_time > self.recalculation_interval):
            ...
            try:
                ...
                # Do not create DB snapshots during backtesting
                if self.state_manager.mode != 'backtest':
...
class TradingBot:
    ...
    def _check_and_handle_refresh_signal(self):
        ...
        # This functionality is disabled in backtest mode
    ...
    def _run_single_cycle(self):
        ...
        try:
            ...
            if features_df.empty:
                ...
                if self.mode in ['trade', 'test']: time.sleep(10)
                return
            ...
            if current_regime == -1:
                ...
                if self.mode in ['trade', 'test']: time.sleep(10) # Wait before retrying
                return
            ...
            if self.is_monitoring_for_reversal:
                ...
                if current_price >= reversal_target_price:
                    ...
                    if self.mode in ['trade', 'test']: time.sleep(30)
                    return
            ...
            # Update the status file at the end of the regular cycle, only if not in backtest mode
            if self.mode != 'backtest':
                self._update_status_file()
            ...
            if self.mode in ['trade', 'test']: time.sleep(60) # Sleep for a bit to avoid spamming logs if the error is persistent
        except KeyboardInterrupt:
            self.is_running = False
            logger.info("\n[SHUTDOWN] Ctrl+C detected.")
        except Exception as e:
            logger.critical(f"❌ Critical error in main loop: {e}", exc_info=True)
            if self.mode in ['trade', 'test']: time.sleep(300)
