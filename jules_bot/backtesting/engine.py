...
class Backtester:
    def __init__(self, db_manager: PostgresManager, days: int = None, start_date: str = None, end_date: str = None, config_manager_override: "ConfigManager" = None):
        self.run_id = f"backtest_{uuid.uuid4()}"
        self.db_manager = db_manager
        self.cm = config_manager_override or config_manager
        ...
        initial_balance_str = self.cm.get('BACKTEST', 'initial_balance') or '1000.0'
        commission_fee_str = self.cm.get('STRATEGY_RULES', 'commission_rate') or '0.001'
        self.mock_trader = MockTrader(
            initial_balance_usd=Decimal(initial_balance_str),
            commission_fee_rate=Decimal(commission_fee_str),
            symbol=symbol
        )
        self.strategy_rules = StrategyRules(self.cm)
        self.capital_manager = CapitalManager(self.cm, self.strategy_rules, db_manager=self.db_manager)

        # --- Dynamic Strategy Components ---
        self.dynamic_params = DynamicParameters(self.cm)
    ...
    def run(self):
        ...
        symbol = self.cm.get('APP', 'symbol')
        strategy_name = self.cm.get('APP', 'strategy_name', fallback='default_strategy')
        min_trade_size = Decimal(self.cm.get('TRADING_STRATEGY', 'min_trade_size_usdt', fallback='10.0'))
        ...
        for current_time, candle in self.feature_data.iterrows():
            ...
            for trade_id, position in list(open_positions.items()):
                ...
                if current_price >= sell_target_price:
                    positions_to_sell_now.append((position, 'take_profit'))
                    continue
                ...
                if decision == "ACTIVATE":
                    ...
                    if current_price > break_even_price:
                        ...
                        positions_to_sell_now.append((position, 'trailing_stop'))
                    else:
                        logger.warning(
            ...
            if positions_to_sell_now:
                for position, sell_reason in positions_to_sell_now:
                    trade_id = position['trade_id']
                    original_quantity = position['quantity']
                    sell_quantity = original_quantity * strategy_rules.sell_factor

                    decision_context_sell = {**candle.to_dict(), 'sell_reason': sell_reason}
                    success, sell_result = self.mock_trader.execute_sell({'quantity': sell_quantity}, self.run_id, decision_context_sell)
                    if success:
                        ...
                        trade_data = {
                            ...
                            'trade_id': trade_id, 'linked_trade_id': trade_id,
                            ...
                            'commission_asset': "USDT", 'timestamp': current_time, 'decision_context': decision_context_sell,
        ...
        summary = self._generate_summary(open_positions, portfolio_history)
    ...
    def _generate_summary(self, open_positions: dict, portfolio_history: list[Decimal]) -> dict:
        ...
        for col in ['price', 'quantity', 'usd_value', 'commission', 'commission_usd', 'realized_pnl_usd']:
        ...
        avg_gain = Decimal(str(winning_trades['realized_pnl_usd'].mean())) if len(winning_trades) > 0 else Decimal(0)
        avg_loss = Decimal(str(abs(losing_trades['realized_pnl_usd'].mean()))) if len(losing_trades) > 0 else Decimal(0)
        payoff_ratio = (avg_gain / avg_loss) if avg_loss > 0 else Decimal(0)

        # Sell Quality Analysis
        def get_sell_reason(context):
            if isinstance(context, dict):
                return context.get('sell_reason', 'unknown')
            return 'unknown'

        sell_trades['sell_reason'] = sell_trades['decision_context'].apply(get_sell_reason)
        take_profit_sells = sell_trades[sell_trades['sell_reason'] == 'take_profit'].shape[0]
        trailing_stop_sells = sell_trades[sell_trades['sell_reason'] == 'trailing_stop'].shape[0]

        # Risk/Return Metrics
        max_drawdown = Decimal(0)
        if portfolio_history:
            portfolio_series = pd.Series(portfolio_history, dtype=float)
            returns = portfolio_series.pct_change().dropna()

            # Calculate Sharpe Ratio (annualized)
            # Assuming daily data for simplicity in calculating trading days
            trading_days = len(self.feature_data)
            annualization_factor = np.sqrt(365) if trading_days > 1 else 1
            sharpe_ratio = (returns.mean() / returns.std()) * annualization_factor if returns.std() != 0 else 0

            # Calculate Sortino Ratio (annualized)
            downside_returns = returns[returns < 0]
            downside_std = downside_returns.std()
            sortino_ratio = (returns.mean() / downside_std) * annualization_factor if downside_std != 0 else 0
        ...
        summary = {
            "Run ID": self.run_id,
            "Start Date": self.feature_data.index[0].date(),
            "End Date": self.feature_data.index[-1].date(),
            "Initial Balance": float(initial_balance),
            "Final Balance": float(final_balance),
            "Net P&L": float(net_pnl),
            "Net P&L %": float(net_pnl_percent),
            "Realized PnL": float(total_realized_pnl),
            ...
            "Avg Gain $": float(avg_gain),
            "Avg Loss $": float(avg_loss),
            "Sharpe Ratio": sharpe_ratio,
            "Sortino Ratio": sortino_ratio,
            "Max Drawdown %": float(max_drawdown * 100),
            "Fees Paid $": float(total_fees_usd),
            "Take Profit Sells": take_profit_sells,
            "Trailing Stop Sells": trailing_stop_sells
        }
